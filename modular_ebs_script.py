...
def find_unattached_volumes(clusterAZ, instanceEnv, instanceApp, extraFilters, client):
    ...
    while True:
        try:
            yourVolumes = client.describe_volumes(Filters=filters) #{'Name': 'status', 'Values':['available']}
            break
        except ClientError as e:
            if e.response['Error']['Code'] == 'RequestLimitExceeded' and attempts < 10:
                sleep(random.random()*(min(300, 2*2**attempts)))
    ...
    startVolume = None
    for volume in yourVolumes['Volumes']:
...
def main(args):
    ...
    instanceEnvironment = instanceTags['Environment'] ###GET CLARIFICATION ON THIS ONE
    instanceApplication = instanceTags['Application']
    instanceName = instanceTags['Name']
    instanceIP = client.describe_instances(InstanceIds=[instanceID])['Instances'][0]['PrivateIpAddress']
    dataPaths = {'blockDevice': 'blk_dev_start', 'numInSet': 'num_vol_set', 'size':'', 'volType':'', 'iops':'', 'commitLogSize':''}
    ...
    cloudwatchClient.put_metric_data(Namespace='instanceID', MetricData=[{'MetricName': 'instanceStatus', 'Value': 1}])
...
p = subprocess.check_output(cmd, shell=True)
    ...
    tag_volumes(volumeIDsToAttach, blockDevices, mountPoints, numInSet, instanceEnv, instanceApp, instanceName, instanceIP, client)
    return 0
